...
#### Why use the `__invoke` method?

The request handler uses `__invoke` to indicate that the handler should only
have a single purpose: to transform the request into a response. Using `__invoke`
implies that the request handler will be used in a functional way.

Additionally, due to [parameter type widening][param-widening] the handler MAY
be replaced by another callable that serves the same purpose.

[param-widening]: https://wiki.php.net/rfc/parameter-no-type-variance
...
#### Why isn't the handler typed as `callable`?

Despite the fact that the request handler uses the `__invoke` method, it SHOULD
NOT be typed as a `callable`. Using the interface type improves runtime safety
and IDE support.
